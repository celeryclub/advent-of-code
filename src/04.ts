...
function part1() {
  ...
  events.forEach(event => {
    ...
    if (shiftChangeEvent) {
      ...
      if (currentGuardSleepStartTime !== null) {
        ...
        guardNaps[currentGuardId] =
          (guardNaps[currentGuardId] ? guardNaps[currentGuardId] : []).concat([newSleep]);
  ...
  for (var guardId in guardNaps) {
    const napLength = guardNaps[guardId].map(nap => {
      return nap[1] - nap[0];
  ...
  const allNapMinutes = guardNaps[sleepiestGuardId].map(nap => {
